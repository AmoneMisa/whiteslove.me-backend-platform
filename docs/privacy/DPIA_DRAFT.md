...
|---|---|---|---|---|---|---|
| R1 | People are profiled without knowing it | Public-source collection, Art 14 | High | High | Operator decision: Art 14(5)(b) exemption with documented reasoning; public privacy notice and data-rights page | `/privacy` and `/data-rights` are built (RU and EN); keep them deployed and accurate, since the exemption does not hold without them |
...
| R6 | Personal data sent to AI providers, possibly used for training or transferred abroad | Processors, Chapter V | High | Medium | Contacts redacted from extraction prompts; contacts masked in translation requests (free translator and LLMs) and restored afterwards | **Photos are sent to vision providers.** Confirm enabled providers, their data-use terms and transfer mechanisms; consider excluding photos showing people or documents |
...
| R10 | Objection or restriction recorded but not honoured | Rights | High | Medium | Flags stored and audited. Enforced in the identity repository used by ingestion: restricted contacts are not observed again, no links or roles are recorded for restricted or objecting subjects, and `findRestrictedContactPoints` tells an ingest which contacts to skip entirely. Evidence is loaded with `processingRestricted`, so public states and high-impact actions exclude it; listing lines and owner collections skip these subjects | Any future ingest must call `findRestrictedContactPoints` before creating actors |
...
- Prior consultation with a supervisory authority (Art 36): required if high
  residual risk remains after measures. **Cannot be assessed until R1 and R6 are
  addressed.**

## 7. Outcome

**Not complete.** Identity and risk features should not be enabled in
production until R1 and R6 are resolved, this document is reviewed,
