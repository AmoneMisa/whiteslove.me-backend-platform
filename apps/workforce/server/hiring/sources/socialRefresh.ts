...
type SocialTarget = { key: string; label: string; platform: SocialPlatform; country: 'UZ' | 'KZ' | 'KG' | 'UA' | 'RO'; city?: string; target?: string; query?: string }
type SocialItem = { id?: string; author?: string; text?: string; url?: string; createdAt?: string | null; images?: string[] }
type SocialResponse = { ok?: boolean; count?: number; items?: SocialItem[]; boundaryReached?: boolean; exhausted?: boolean; error?: string }
...
export function hiringSocialSourceHandles(): string[] { return configuredTargets().map((target) => `social:${target.key}`) }
export function listSocialSources(): Array<{ key: string; label: string; country: string; origin: SocialPlatform }> { return configuredTargets().map((target) => ({ key: target.key, label: target.label, country: target.country, origin: target.platform })) }

function cutoffTime(): number { const date = new Date(); date.setUTCMonth(date.getUTCMonth() - MAX_AGE_MONTHS); return date.getTime() }
function recentIso(value: string | null | undefined): string | null { if (!value) return null; const time = Date.parse(value); if (!Number.isFinite(time) || time < cutoffTime() || time > Date.now() + 48 * 60 * 60 * 1000) return null; return new Date(time).toISOString() }
function contacts(text: string, country: SocialTarget['country']): DirectContacts { return { ...extractCandidateContacts(text, country) } }
...
function roleFrom(text: string): string { const intent = text.split('\n').map((line) => line.replace(/\s+/g, ' ').trim()).find((line) => /(?:ищу|шукаю).{0,20}(?:работ|робот)|(?:ish.{0,20}(?:kerak|qidir|izlay))|жұмыс\s+іздеймін|жумуш\s+(?:издейм|издеп)|caut\s+(?:un\s+)?loc\s+de\s+munc|(?:îmi|imi)\s+caut\s+(?:un\s+)?job/iu.test(line)); const role = String(intent || '').replace(INTENT_PREFIX_RE, '').replace(UZ_INTENT_PREFIX_RE, '').replace(LOCAL_CANDIDATE_INTENT_RE, '').replace(/^\s*[:—-]\s*/, '').trim(); return role.length >= 2 && role.length <= 180 ? role : '' }
function nameFrom(item: SocialItem, text: string): string { const explicit = text.match(/(?:^|\n)\s*(?:имя|ім['’]я|ism(?:i|im)?|name|nume)\s*[:—-]\s*([^\n]{2,80})/iu)?.[1]?.trim(); if (explicit && !/ваканс|компан|работ|робот|ish|job/iu.test(explicit)) return explicit; const author = String(item.author || '').trim(); return author && author.length <= 80 && !/^\d+$/.test(author) ? author : '' }
...
function itemToProfile(item: SocialItem, target: SocialTarget): CvProfile | null {
  ...
  return normalizeCandidate({ id: `social-${target.key}-${id}`, source: 'social', origin: target.platform, sourceKey: target.key, sourceLabel: target.label, sourceCountry: target.country, country: target.country, name: nameFrom(item, text), role, professions: role ? [role] : [], city: detectCity(text, target.country) || target.city || null, isAdult: true, remote: detectCandidateRemotePreference(text), url: item.url, publishedAt: createdAt, updatedAt: createdAt, activityAt: createdAt, createdAt, originalText: text.slice(0, 4_000), description: text.slice(0, 4_000), photos: Array.isArray(item.images) ? item.images.slice(0, 8) : [], photo: Array.isArray(item.images) ? item.images[0] || null : null, contacts: publicContacts, contact: direct || item.url, contactType: direct ? 'direct' : 'platform', tags: [target.label, target.platform === 'facebook' ? 'Facebook' : 'Threads', target.country] })
}

async function fetchTarget(target: SocialTarget): Promise<{ profiles: CvProfile[]; fetched: number; reachedBoundary: boolean }> {
  const cutoff = new Date(cutoffTime()).toISOString()
  const payload = target.platform === 'facebook' ? { source: 'facebook', target: target.target, cutoff } : { source: 'threads', mode: 'search', query: target.query, cutoff }
  const response = await fetch(`${socialFetcherBaseUrl()}/crawl`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  return { fetched: Number.isFinite(body.count) ? Number(body.count) : items.length, reachedBoundary: Boolean(body.boundaryReached || body.exhausted), profiles: items.map((item) => itemToProfile(item, target)).filter((item): item is CvProfile => Boolean(item)) }
}
...
export async function refreshHiringSocialSource(handle: string): Promise<{ fetched: number; candidates: number; stored: number } | null> {
  ...
  const base = { handle: `social:${target.key}`, key: `social:${target.key}`, label: target.label, country: target.country, pages: 1, duplicate: 0, expired: 0, shown: 0, newestActivityAt: null, oldestActivityAt: null, lastSeenProfileId: '', lastSuccessAt: null, reachedCursor: false, checkedAt }
  try {
    const run = await fetchTarget(target)
    const times = run.profiles.map((profile) => profile.createdAt || '').filter(Boolean).sort()
    const diagnostic: WebSourceDiagnostic = { ...base, status: run.profiles.length ? 'ok' : 'empty', fetched: run.fetched, candidates: run.profiles.length, blocks: run.fetched, parsed: run.profiles.length, rejected: Math.max(0, run.fetched - run.profiles.length), fetchDurationMs: Date.now() - startedAt, newestActivityAt: times.length ? times[times.length - 1]! : null, oldestActivityAt: times[0] || null, lastSuccessAt: checkedAt, reachedCursor: run.reachedBoundary }
    const persisted = await persistWebProfiles(run.profiles, diagnostic, target.key)
    diagnostic.shown = persisted.shown; diagnostic.expired = persisted.expired; recordWebDiagnostic(diagnostic)
    ...
    recordWebDiagnostic({ ...base, status: 'error', fetched: 0, candidates: 0, blocks: 0, parsed: 0, rejected: 0, fetchDurationMs: Date.now() - startedAt, error: (error as Error).message })
    throw error
  }
}
