-- Second half of the roomOnly/commercial/listingKind/listingStatus promotion
-- started in 038: rewrite the indexes and trigger function that used to
-- re-derive these values from data on every row to use the new generated
-- columns instead. Run after 038 commits so the ACCESS EXCLUSIVE heap
-- rewrite there doesn't overlap this migration's longer index-build phase
-- (same reasoning as 024 -> 026).

-- These four raw-listings partial indexes (from 011_public_feed_fast_path.sql
-- and 012_public_feed_recency_index.sql) show zero scans in production
-- (pg_stat_user_indexes) -- the public-feed read path has moved onto
-- listing_public_feed_members/listing_public_feed_canonical since. Drop
-- rather than rewrite: removes dead per-row index-maintenance overhead
-- instead of just making it cheaper.
DROP INDEX IF EXISTS listings_public_feed_dedupe_created_idx;
DROP INDEX IF EXISTS listings_public_feed_country_dedupe_created_idx;
DROP INDEX IF EXISTS listings_public_feed_created_dedupe_idx;
DROP INDEX IF EXISTS listings_public_feed_country_created_dedupe_idx;

-- listings_market_rooms_expr_idx / listings_market_area_expr_idx
-- (015_market_comparison_expression_indexes.sql): same shape, swap the jsonb
-- expressions for the generated columns.
DROP INDEX IF EXISTS listings_market_rooms_expr_idx;
...
DROP INDEX IF EXISTS listings_market_area_expr_idx;
...
-- sync_listing_public_feed_member() (currently defined in
-- 034_public_feed_search_read_model.sql): swap the jsonb-derived entry
-- condition and v_room_only computation for the generated columns. Safe
-- because generated columns are recomputed after BEFORE ROW triggers run
-- (canonicalize_listing_room_rent, which writes NEW.data) and before AFTER
-- ROW triggers (this one) fire, so NEW.room_only/commercial/listing_kind/
-- listing_status already reflect any data mutation made earlier in the same
-- statement. Every other field in this function (new_building, dishwasher,
-- ...has_photos) is unchanged.
