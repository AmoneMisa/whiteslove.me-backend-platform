...
export async function geocodeListingsPersistent(listings, country) {
  ...
  for (const listing of listings) {
    ...
    if (hasCoordinates(listing)) {
      const exactAddressRefined = await refineSourceCoordinateFromExactAddress(listing, country, candidates, budget);
      if (!exactAddressRefined && refineSourceCoordinateFromGeoCatalogResidentialComplex(listing, country)) {
