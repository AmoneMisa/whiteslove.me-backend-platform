...
test('HTTP route modules never import custom scraper execution helpers', () => {
  for (const name of ['routes/listing-routes.js', 'routes/listing-item-routes.js', 'app.js', 'server.js']) {
...
test('listing freshness policy is imported directly instead of re-exported by normalize', () => {
  const normalize = readFileSync(path.join(srcRoot, 'listing/normalize.js'), 'utf8');
  const legacyFilter = readFileSync(path.join(srcRoot, 'legacy/legacy-listing-filter.js'), 'utf8');
  ...
  assert.match(legacyFilter, /from '\.\.\/listing\/listing-policy\.js'/);
  assert.match(telegram, /from '\.\.\/listing\/listing-policy\.js'/);
  assert.match(social, /from '\.\.\/listing\/listing-policy\.js'/);
});
...
test('listing normalization consumes aggregate package-owned fields', () => {
  const source = readFileSync(path.join(srcRoot, 'listing/normalize-legacy.js'), 'utf8');
...
test('source adapters consume package-owned housing semantics', () => {
  for (const name of [
    'scheduling/queueTasks.js',
    'legacy/legacy-listing-filter.js',
    'scrapers/custom.js',
    'scrapers/olx.js',
