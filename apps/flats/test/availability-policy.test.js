...
test('OLX availability selection and verification share one TTL policy', async () => {
  const [policy, verifier, sweep] = await Promise.all([
    readFile(new URL('../src/availability/availability-policy.js', import.meta.url), 'utf8'),
    readFile(new URL('../src/availability/availability.js', import.meta.url), 'utf8'),
    readFile(new URL('../src/availability/availability-sweep.js', import.meta.url), 'utf8'),
  ]);
